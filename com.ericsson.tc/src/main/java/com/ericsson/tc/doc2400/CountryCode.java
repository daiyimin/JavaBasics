...
/**
 * Country code is defined in: <br/>
 * http://anon.ericsson.se/eridoc/component/eriurl?docno=EAB-12:038373Uen&
 * objectId=09004cff85a921da&action=approved&format=msw8
 * 
 * @author eyimdai
 *
 */
public class CountryCode {
	public static final String[][] COUNTRY_CODE = { { "AFGHANISTAN ", "AF" },
			{ "Aland Islands", "AX" }, { "ALBANIA", "AL" },
			{ "ALGERIA", "DZ" }, { "AMERICAN SAMOA", "AS" },
			{ "ANDORRA", "AD" }, { "ANGOLA", "AO" }, { "ANGUILLA", "AI" },
			...
			{ "BHUTAN", "BT" }, { "BOLIVIA", "BO" }, { "BONAIRE", "BQ" },
			{ "BOSNIA AND HERZEGOWINA", "BA" }, { "BOTSWANA", "BW" },
			...
			{ "COTE D'IVOIRE", "CI" },
			{ "CROATIA (local name: Hrvatska)", "HR" }, { "CUBA", "CU" },
			{ "CURACAO", "CW" }, { "CYPRUS", "CY" },
			...
			{ "GUAM", "GU" }, { "GUATEMALA", "GT" }, { "GUERNSY", "GG" },
			...
			{ "MEXOCO", "MX" }, { "MICRONESIA, FEDERATED STATES OF", "FM" },
	...
	public static boolean compareCountryCode(String left, String right) {
		boolean match = false;
		for (int i = 0; i < COUNTRY_CODE.length; i++) {
			String countryName = COUNTRY_CODE[i][0].trim().toLowerCase();
			String countryCode = COUNTRY_CODE[i][1].trim().toLowerCase();

			String leftVal = left.trim().toLowerCase();
			String rightVal = right.trim().toLowerCase();

			if (countryCode.equals(leftVal) || countryName.equals(leftVal)) {
				if (countryCode.equals(rightVal)
						|| countryName.equals(rightVal)) {
					match = true;
					break;
				}
			}
